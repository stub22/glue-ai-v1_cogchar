...
public class BitCube extends AbstractBitGoody {
	...
	public BitCube(RenderRegistryClient aRenderRegCli, Ident boxUri, Vector3f initialPosition, Quaternion initialRotation,
	...
	public void setState(boolean boxState) {
		if (boxState != state) {
			...
			moveViaAnimation(myPosition, new Quaternion().fromAngles(0f, (float)Math.PI, 0f), myScale, STATE_TRANSITION_TIME);
			setNewGeometryRotationOffset(geomIndex, getStateAdjustedRotationOffset(boxState));
