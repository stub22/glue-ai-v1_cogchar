...
public class SoundDetector {
	private static final Logger theLogger = LoggerFactory.getLogger(SoundDetector.class);
	...
	public boolean initialize() {
		...
		try {
			...
			theLogger.warn("Unable to open microphone.", ex);
			return false;
		}
	}
	...
	public boolean start() {
		if (!myInitializedFlag) {
			if (!initialize()) {
				theLogger.warn("Unable to initialize microphone.");
				return false;
			}
		}
		if (myCaptureThread != null) {
			...
			theLogger.warn("Unable to start CaptureThread, "
	...
	public void longSilenceDetected() {
		...
		while (myDicationGrabber.collectDictation()) {
			update = true;
			try {
				Thread.sleep(myDetectConfig.getSendWaitLength());
		...
		public void run() {
			...
			while (myRunFlag) {
				try {
					...
					theLogger.warn("Error in sound capture thread.", t);
				}
			}
