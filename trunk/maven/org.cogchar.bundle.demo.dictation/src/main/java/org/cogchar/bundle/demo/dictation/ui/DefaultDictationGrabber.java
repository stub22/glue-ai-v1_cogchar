...
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_BROKER_CLIENT_NAME;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_BROKER_IP;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_BROKER_PASSWORD;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_BROKER_PORT;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_BROKER_USERNAME;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_BROKER_VIRTUAL_HOST;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.CONF_DESTINATION;
import static org.cogchar.bundle.demo.dictation.osgi.DictationConfigUtils.getValue;

/**
 * @author Matthew Stevenson <www.cogchar.org>
 */
public class DefaultDictationGrabber implements DictationGrabber {
	private static final Logger theLogger = LoggerFactory.getLogger(DefaultDictationGrabber.class);
	...
	public DefaultDictationGrabber(
		...
		myDictationLogger = new DefaultNotifier<>();
		myConcatInputNotifier = new DefaultNotifier<>();
	}
	...
	public boolean connect() {
		...
		try {
			...
			theLogger.warn("Error building speech sender.", ex);
			return false;
		}
	}
	...
	private ConsumerNode<String> buildSpeechSenderNodeChain() throws JMSException {
		...
		try {
			JMSAvroMessageSender<SpeechRecEventList, SpeechRecEventListRecord> sender =
					new JMSAvroMessageSender<>(mySession, dest);
			sender.setAdapter(new EmptyAdapter());
			sender.setDefaultContentType("application/speechRecEventList");
			sender.start();
			ConsumerNode<SpeechRecEventList> ttsNode = new DefaultConsumerNode<>(sender);
