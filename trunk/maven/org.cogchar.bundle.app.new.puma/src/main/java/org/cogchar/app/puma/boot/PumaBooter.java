...
public class PumaBooter extends BasicDebugger {
    ...
    /**
     * Public entry point for the PUMA application system when running in an
     * OSGi environment. Presumes that an SLF4J logging binding is already in
     * place. Normally this is *not* called directly from another bundle's
     * activator. Instead, it is called after all initial bundles (for your
     * application) are loaded, which is signaled in a FrameworkEvent
     *
     * @param bundleCtx - an OSGi bundle
     * @param mediator - users customize this object to configure/influence the
     * boot process.
     * @return a BootResult indicating the status of boot attempt. No app data
     * is passed here.
     */
    public BootResult bootUnderOSGi(BundleContext bundleCtx, PumaContextMediator mediator) {
        ...
        ServiceLifecycleProvider<PumaContextMediator> lifecycle =
                new SimpleLifecycle<PumaContextMediator>(mediator, PumaContextMediator.class.getName());
        Properties props = new Properties();
        props.put("pumaMediator", "pumaMediator");

        ManagedService<PumaContextMediator> ms = new OSGiComponent<PumaContextMediator>(bundleCtx, lifecycle, props);
        ms.start();
        return result;
    }

    /**
     * Protected initialization of an OSGi context - the guts. Normally this is
     * called from bootUnderOSGi, which is called from a
     * FrameworkStarted-EventHandler (NOT the BundleActivator.start() method !)
     * of some "top" application bundle client, e.g. Friendularity
     * CCRK_DemoActivator. That client controls our boot process through
     * supplied mediator.
     *
     * @param bundleCtx
     * @param mediator
