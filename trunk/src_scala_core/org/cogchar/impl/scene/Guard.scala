  ...
  override def isSatisfied(scn: BScene): Boolean = {
    ...
    scn match {
      case fbs: FancyBScene => {
        ...
        var aSpecIsGoodAsTheRealThingHere: ThingActionSpec = null

        if (filterToCheck.test(aSpecIsGoodAsTheRealThingHere)) {
          getLogger().debug("ThingAction {} is a MATCH for TA-Filter {}", aSpecIsGoodAsTheRealThingHere, filterToCheck)
          true
        } else {
          getLogger().debug("ThingAction {} is NOT a match for TA-Filter {}", aSpecIsGoodAsTheRealThingHere, filterToCheck)
          false
        }
      }
      case _ => {
