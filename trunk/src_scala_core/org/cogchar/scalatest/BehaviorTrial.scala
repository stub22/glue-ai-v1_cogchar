...
class  WorkThing {
	...
	val sillyActor = actor {
		...
		def emoteLater() {
			val mainActor = self
			log("inside emoteLater, looked up self as mainActor= " + mainActor + ", now starting NEW innerActor")
			val innerActor = actor 
			{
				log("innerActor[" + self + "] has been created, and will now sleep for 1 sec")
				Thread.sleep(1000)
		...
		log("sillyActor - calling emoteLater() for the first time")
		emoteLater()
...
object BehaviorTrial {
	...
	def main(args: Array[String]) {
		...
		log("sillyActor=" + wt.sillyActor)
		log("main() sleeping for 10 sec")
		Thread.sleep(2500)
		log("main sending to sillyActor")
		wt.sillyActor ! "Hey, this message is from main(), and I am the boss of you, mr SillyActor"
		log("main END ------------------- ")
	}  
}
