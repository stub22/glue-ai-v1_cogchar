...
object ToggleButton extends AbstractControlInitializationHandler {
  ...
  protected def handleHere(sessionId:String, slotNum:Int, control:ControlConfig): NodeSeq = {
	...
	val buttonState = LifterVariableHandler.getStateFromVariable(sessionId, control.action)
	...
	val myControlDef = PageCommander.getState.controlDefMap(sessionId)(slotNum)
	myControlDef.action =
