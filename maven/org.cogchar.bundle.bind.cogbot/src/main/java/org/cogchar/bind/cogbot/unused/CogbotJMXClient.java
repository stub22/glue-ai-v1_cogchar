...
import static org.cogchar.bind.cogbot.main.CogbotConfigUtils.CONF_COGSIM_JMX_READY;
...
public class CogbotJMXClient implements NotificationListener, NotificationFilter {
	private static final Logger theLogger = LoggerFactory.getLogger(CogbotJMXClient.class);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					echo("connectJMX: ", ex);
					org.slf4j.LoggerFactory.getLogger(CogbotJMXClient.class).error(ex.getMessage(), ex);
				}
			}
		}).start();
	}
