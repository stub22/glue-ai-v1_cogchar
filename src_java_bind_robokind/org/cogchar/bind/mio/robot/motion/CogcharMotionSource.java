 ...
 * Each instance binds to the RK joint-frame protocol for one jointed robot. Holds an ordered list
 * of joint computers, each of which receives one notifySourceComputingCycle() each time we process
 * getMovements(). No protocol among those computers is enforced at this level, we simply presume
 * they will somehow call our "move()" method (inherited from RobotMoverFrameSource) in a sensible
 * way, with the last joint-computer in our collection having the last word. So, this class provides
 * capability, without policy, and is a good place to begin investigating your own custom low-level
 * (in Cogchar terms) movement planning systems.
 *
 * Under OSGi (or other JFlux binding), instances of CogcharMotionSource can be found in the
 * registry using findCogcharMotionSources, which can be extended to use robotID as a filter
 * parameter.
 */
public class CogcharMotionSource extends RobotMoverFrameSource {
    ...
    @Override public synchronized MotionFrame getMovements(long currentTimeUTC,
    ...
    @Override public synchronized void move(RobotPositionMap positions, long lenMillisec) {
