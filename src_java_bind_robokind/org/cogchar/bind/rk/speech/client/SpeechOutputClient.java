...
**/
	...
	@Override public void fancyFastCueAndPlay (FancyTextMedia ftm, FancyTextCursor cuePos, FancyTextPerf perf)  {
	...
	/*
>     On 4/8/2013 5:55 PM, Matt wrote:
>         I just changed the SpeechService so the speak(String) method now returns a SpeechJob.
>         The SpeechJob has addListener(Listener<org.__jflux.api.core.playable.__Playable.PlayState>), this is
>         also a PlayState in robokind which you do not want to use.
>         You will be notified when the speech begins with RUNNING.  There can be a delay between calling
>         speak(String)
>         and the speech job starting if other text is already in the speech queue.  You may miss the RUNNING
>         notification by the time you get a SpeechJob and are able to add the listener.
>         When the speech is complete you will be notified with COMPLETED, and if it is canceled you will be
>         notified with ABORTED.
> On Mon, Apr 8, 2013 at 7:23 PM, StuB22
* >  >     So, possibly on a very short or empty speech job (e.g. "") , we could also
>     miss the "COMPLETED" notice?> 
>     If so, can we additionally synchronously query the SpeechJob for its status?
      
	 * On 4/8/2013 6:31 PM, Matt Stevenson wrote:> Yes, there is a getStatus() method.  It returns an int for a 
	 * status as defined in DefaultSpeechJob (this was copied over from Robosteps).  I will leave it this way 
	 * for the show and clean it up a little more next week.
> I could also modify the speech job so if it is completed or canceled and a listener is added, it will be 
> notified immediately.
> 
> Another potential problem is sending a speech command at the same time from a different instance.  The speech 
> jobs would get confused about the events from the other commands, but you can send two speech commands from a 
> single instance without a problem.
> 

//	Here are the methods we can call on SpeechEvent
